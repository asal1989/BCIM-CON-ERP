...
const { sendPushToUsers, sendPushToRole } = require('../services/pushNotification.service');
...
async function createNotification(opts) {
  ...
  // Push fan-out — every notification (approval, alert, etc.) also tries to
  // reach the user's phone, not just the in-app bell. Fire-and-forget so a
  // push failure can never break or delay the parent request.
  const pushPayload = {
    title: opts.title,
    body: opts.message || '',
    data: { link: opts.link || null, type: opts.type || null, related_type: opts.related_type || null, related_id: opts.related_id || null },
  };
  if (opts.user_id) {
    sendPushToUsers([opts.user_id], pushPayload).catch(() => {});
