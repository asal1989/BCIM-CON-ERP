...
router.post('/', async (req, res) => {
  try {
    ...
    // Notify PO approvers (Stage 1: procurement_manager, project_manager, admin, super_admin) —
    // in-app + push, same as the existing WhatsApp/email fan-outs below.
    for (const role of ['procurement_manager', 'project_manager', 'admin', 'super_admin']) {
      createNotification({
        company_id: req.user.company_id,
        target_role: role,
        type: 'po_pending_approval',
        title: 'New PO awaiting approval',
        message: `PO ${result.po_number} needs your approval.`,
