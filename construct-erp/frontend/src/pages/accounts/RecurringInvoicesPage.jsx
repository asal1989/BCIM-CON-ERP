// src/pages/accounts/RecurringInvoicesPage.jsx
...
import { recurringInvoiceAPI, projectAPI } from '../../api/client';
...
function ProfileForm({ onClose }) {
  ...
  const [form, setForm] = useState({
    profile_name: '', client_name: '', project_id: '',
    frequency: 'monthly', day_of_month: '1', next_run_date: dayjs().format('YYYY-MM-DD'),
    basic_amount: '', gst_pct: '18', description: '',
  });
  ...
  const saveMut = useMutation({
    mutationFn: () => recurringInvoiceAPI.create(form).then(r => r.data),
    onSuccess: () => { toast.success('Recurring invoice profile created'); qc.invalidateQueries({ queryKey: ['recurring-invoices'] }); onClose(); },
      ...
      <div className="bg-white w-full max-w-lg rounded-md border border-slate-200 shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-md bg-blue-50 flex items-center justify-center"><ScrollText className="w-4 h-4 text-blue-600" /></div>
            <p className="text-sm font-semibold text-black">New Recurring Invoice Profile</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div><Lbl req>Profile Name</Lbl><input className={F} placeholder="e.g. Monthly Retainer — Divyasree" value={form.profile_name} onChange={e => set('profile_name', e.target.value)} /></div>
          ...
          <div><Lbl req>Amount per Invoice (₹, basic)</Lbl><input type="number" step="0.01" className={F} value={form.basic_amount} onChange={e => set('basic_amount', e.target.value)} /></div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saveMut.isPending} className="flex-1 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 disabled:opacity-50">
...
function HistoryModal({ profile, onClose }) {
  const { data: log = [], isLoading } = useQuery({
    queryKey: ['recurring-invoice-log', profile.id],
    queryFn: () => recurringInvoiceAPI.log(profile.id).then(r => r.data?.data || []),
  });
  return (
    <div className="fixed inset-0 z-[70] bg-black/40 flex items-center justify-center p-4 overflow-auto">
        ...
        <div className="p-5 space-y-2 max-h-96 overflow-y-auto">
          ...
          : log.map(l => (
            <div key={l.id} className="flex items-center justify-between px-3 py-2 border border-slate-100 rounded-md">
              <div>
                <p className="text-sm font-mono font-semibold text-black">{l.invoice_no}</p>
                <p className="text-xs text-slate-400">{dayjs(l.invoice_date).format('DD MMM YYYY')}</p>
              </div>
              <p className="text-sm font-mono font-semibold text-emerald-700">{inr(l.total_amount)}</p>
            </div>
          ))}
        </div>
      </div>
...
export default function RecurringInvoicesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => recurringInvoiceAPI.list().then(r => r.data?.data || []),
  });
  const rows = data || [];

  const toggleMut = useMutation({
    mutationFn: ({ id, status }) => recurringInvoiceAPI.updateStatus(id, status),
    onSuccess: () => { toast.success('Status updated'); qc.invalidateQueries({ queryKey: ['recurring-invoices'] }); },
    onError: e => toast.error(e?.response?.data?.error || 'Update failed'),
  });
  const generateMut = useMutation({
    mutationFn: (id) => recurringInvoiceAPI.generate(id),
    onSuccess: (res) => { toast.success(`Invoice ${res.data?.data?.invoice_no} generated`); qc.invalidateQueries({ queryKey: ['recurring-invoices'] }); },
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-md bg-blue-50 flex items-center justify-center"><ScrollText className="w-4 h-4 text-blue-600" /></div>
            <div>
              <h1 className="text-lg font-semibold text-black">Recurring Invoices</h1>
              <p className="text-xs text-slate-400">Profiles that generate client invoices on a schedule — click Generate Now when due</p>
            </div>
          </div>
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700">
          ...
          return (
            <div key={r.id} className="bg-white border border-slate-200 rounded-md p-4 flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-xs text-slate-500">{r.client_name} · {FREQ_LABEL[r.frequency]} · Next: {r.next_run_date ? dayjs(r.next_run_date).format('DD MMM YYYY') : '—'} · Generated {r.generated_count || 0}× ({inr(r.generated_total)})</p>
              </div>
