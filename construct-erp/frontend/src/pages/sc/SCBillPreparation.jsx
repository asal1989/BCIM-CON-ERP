...
const STATUS_META = {
  ...
  submitted:    { bg:'bg-blue-100',    text:'text-blue-700',    label:'Waiting for QS Manager Approval' },
  under_review: { bg:'bg-amber-100',   text:'text-amber-700',   label:'Waiting for MD Approval' },
            ...
            {nmrs.map(n => (
              <div key={n.id}
                className={clsx('rounded-xl border-2 transition-all',
                  selectedNmrId === n.id ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200 bg-white')}>
                {/* Selectable card row */}
                <button type="button" onClick={() => setSelectedNmrId(n.id)} className="w-full text-left p-4">
