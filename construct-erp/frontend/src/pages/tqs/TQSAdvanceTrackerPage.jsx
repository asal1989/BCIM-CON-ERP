...
function NewAdvanceModal({ onClose, projects, defaultProjectId }) {
  ...
  const { data: vendors = [] } = useQuery({
    ...
    queryFn: () => tqsAdvanceAPI.lookupVendors({ search: vendorSearch }).then(r => r.data?.data ?? []),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const mutation = useMutation({
    mutationFn: (data) => tqsAdvanceAPI.create(data),
    onSuccess: () => {
...
function ImportExcelModal({ onClose, projects, defaultProjectId }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await tqsAdvanceAPI.importExcel(fd);
      setResult(res.data);
      qc.invalidateQueries({ queryKey: ['tqs-advances'] });
      qc.invalidateQueries({ queryKey: ['tqs-advances-summary'] });
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function TQSAdvanceTrackerPage() {
  ...
  // Only fire setSearchParams when the value ACTUALLY differs — prevents repeated
  // URL updates triggering re-render loop / flicker.
  useEffect(() => {
    if (searchInput === search) return;  // no change → no URL update
    const t = setTimeout(() => {
  ...
  const { data: summary = {} } = useQuery({
    queryKey: ['tqs-advances-summary', projectId],
    queryFn: () => tqsAdvanceAPI.summary({ project_id: projectId || undefined }).then(r => r.data?.data ?? {}),
    staleTime: 5 * 60 * 1000,  // 5 min — was 15s causing constant refetch flicker
  });

  const { data: vouchers = [], isLoading } = useQuery({
    queryKey: ['tqs-advances', projectId, statusFilter, search],
    queryFn: () => tqsAdvanceAPI.list({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => tqsAdvanceAPI.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tqs-advances'] });
      qc.invalidateQueries({ queryKey: ['tqs-advances-summary'] });
  ...
  const resyncMut = useMutation({
    mutationFn: () => tqsAdvanceAPI.resyncFromBills(),
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: ['tqs-advances'] });
      qc.invalidateQueries({ queryKey: ['tqs-advances-summary'] });
      const d = res.data;
  ...
  return (
    <div style={{ background: Theme.pageBg, minHeight: '100vh' }}>
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Bill Tracker' }, { label: 'Advances' }]}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm whitespace-nowrap">
                ...
                <tbody>
                  {vouchers.map((v, i) => (
                    <tr key={v.id}
                      onClick={() => navigate(`/tqs/advances/${v.id}`)}
